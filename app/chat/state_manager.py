...
class ConversationStateManager:
    ...
    STATE_TRANSITIONS = {
        BotState.IDLE: [BotState.AWAITING_FILE, BotState.AWAITING_CONFIRMATION, BotState.AWAITING_PARAMETERS],
        BotState.AWAITING_FILE: [BotState.AWAITING_CONFIRMATION, BotState.CANCELLED, BotState.IDLE],
        BotState.AWAITING_CONFIRMATION: [BotState.PROCESSING, BotState.AWAITING_PARAMETERS, BotState.CANCELLED, BotState.IDLE],
        BotState.AWAITING_PARAMETERS: [BotState.AWAITING_CONFIRMATION, BotState.CANCELLED, BotState.IDLE],
    ...
    def transition_state(
        ...
        logger.info(
            f"State transition for {chat_id}: {current_state.value} -> {new_state.value}"
        )
        ...
        for chat_id, context in self._contexts.items():
            if context.last_message_time:
                ...
                if age > max_age_seconds and context.current_state in [
                    BotState.COMPLETED, BotState.FAILED, BotState.CANCELLED
                ]:
